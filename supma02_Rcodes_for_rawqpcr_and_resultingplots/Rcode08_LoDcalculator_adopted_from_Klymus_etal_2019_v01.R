...
# 
# Data Availability Statement: The R script code can be found at https://github.com/cmerkes/qPCR_LOD_Calc (https://doi.org/10.5066/P9GT00GB), and our additional analysis code can be found at https://github.com/cmerkes/LOD_Analysis (https://doi.org/10.5066/P9G4MPVQ). The data used in this study are available at https://doi.org/10.5066/P9AKHU1R.
...
wd02.2 <- "plotout08_LoD_calculations_as_by_Klymus_etal_2019"
...
# select just one file to read in
inf01_txt <- txt.rep.files[2]
# try reading in data from:
# https://github.com/cmerkes/qPCR_LOD_Calc/blob/master/Data.csv
 #inf01_txt<- "/home/hal9000/Documents/Documents/MS_amphibian_eDNA_assays/qPCR_LOD_Calc/LOD_data_from_github.tsv"
# read in file
DAT <- read.csv(inf01_txt, sep="\t", stringsAsFactors = F)
#View(DAT)
# change directory to write out file to the results directory
setwd(wd03)
#modify column names to end up with
#the columns "Target", "Cq", "SQ", as required for this Rcode from Klymus et al. 2019

DAT$Cq <- DAT$Ct..dRn.
DAT$SQ <- DAT$Quantity..copies.
DAT$Target <- DAT$Well.Type
# #make numeric
DAT$SQ <- as.numeric(DAT$SQ)
DAT$Cq <- as.numeric(DAT$Cq)
# #unique(DAT$Target)
# exclude non-standards
DAT <- DAT[(DAT$Target=="Standard"),]
# #Replace NaN & Inf with NA
# # https://www.statology.org/r-error-in-lm-fitx-na-nan-inf-in-y/
DAT[is.na(DAT) | DAT=="Inf"] = NA
#DAT$Cq[is.na(DAT$Cq)] <- 0
...
if(sum(colnames(DAT)=="Target")!=1) { #Is there a "Target" column?
  ...
  if(length(A)!=1) { write("There is a problem with the 'Target' column.\n\n",file="Analysis_Log.txt",append=TRUE) } #Add error message to analysis log.
...
if(sum(is.na(DAT$SQ))>0) {
  ...
  suppressWarnings(write.table(head(DAT[is.na(DAT$SQ),]),file="Analysis_Log.txt",append=TRUE,
...
for(i in 1:length(Targets)) {
  ...
  if(length(unique(DAT$SQ[DAT$Target==Targets[i]]))!=length(STDS2)) {
    ...
    write(ToWrite,file="Analysis_Log.txt",append=TRUE)
  }
...
write("Data Summary:",file="Analysis_Log.txt",append=TRUE)
suppressWarnings(write.table(DAT2,file="Analysis_Log.txt",append=TRUE,sep="\t",eol="\n",
                             row.names=FALSE,col.names=TRUE))
...
write("Assay summary:",file="Analysis_Log.txt",append=TRUE)
write("\nR.squared: The R-squared value of linear regression of all standards Cq-values vs log10 of the starting quantities.",file="Analysis_Log.txt",append=TRUE)
write("Slope: The slope of the linear regression.",file="Analysis_Log.txt",append=TRUE)
write("Intercept: The y-intercept of the linear regression.",file="Analysis_Log.txt",append=TRUE)
write("\nLow.95: The lowest standard with at least 95% positive detection.",file="Analysis_Log.txt",append=TRUE)
write("LOD: The 95% limit of detection as determined by probit modeling.",file="Analysis_Log.txt",append=TRUE)
write(paste0("LOQ: The limit of quantification as determined by decay modeling, using the user-selected CV threshold of: ",LOQ.Threshold),file="Analysis_Log.txt",append=TRUE)
write("\nrep2.LOD: The effective limit of detection if analyzing in 2 replicates.",file="Analysis_Log.txt",append=TRUE)
write("rep3.LOD: The effective limit of detection if analyzing in 3 replicates.",file="Analysis_Log.txt",append=TRUE)
write("rep4.LOD: The effective limit of detection if analyzing in 4 replicates.",file="Analysis_Log.txt",append=TRUE)
write("rep5.LOD: The effective limit of detection if analyzing in 5 replicates.",file="Analysis_Log.txt",append=TRUE)
