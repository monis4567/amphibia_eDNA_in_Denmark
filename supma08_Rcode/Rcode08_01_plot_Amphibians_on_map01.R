...
tot_smpl$pchsymb <- (tot_smpl$eval01=="truezerodetect")*1
tot_smpl$pchsymb <- (tot_smpl$eval01=="repl2pos")*1*21

#https://stackoverflow.com/questions/13353213/gradient-of-n-colors-ranging-from-color-1-and-color-2
nspc <- length(unique(amph_smpl02_df$abbr.nm))
nspct <- length(unique(tot_smpl$abbr.nm))
...
col_for_frog_df <- as.data.frame(cbind(unique(amph_smpl02_df$abbr.nm),
                                       colfunc(nspc)))

col_for_all_df <- as.data.frame(cbind(unique(tot_smpl$abbr.nm),
                                       colfunc(nspct)))
#change column names on th df with colours
colnames(col_for_frog_df) <- c("abbr.nm","hexcol")
colnames(col_for_all_df) <- c("abbr.nm","hexcol")
#plot(rep(1,nspc),col=colfunc(nspc),pch=19,cex=3)
#match colur back to df
amph_smpl02_df$pchcol <- col_for_frog_df$hexcol[match(amph_smpl02_df$abbr.nm, col_for_frog_df$abbr.nm)]
tot_smpl$pchcol <- col_for_all_df$hexcol[match(tot_smpl$abbr.nm, col_for_all_df$abbr.nm)]
