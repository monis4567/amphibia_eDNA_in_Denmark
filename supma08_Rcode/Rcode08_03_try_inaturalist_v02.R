...
plt_amp01 <- ggplot(data = amphib01, aes(x = longitude,
  ...
  coord_fixed(xlim = range(amphib01$longitude, na.rm = TRUE),
              ylim = range(amphib01$latitude, na.rm = TRUE)) +
  theme_bw()
# see the plot
#plt_amp01

#make a list of ambhibians
lst_amphb <- c(
  "Bufo bufo",
  "Bombina bombina",
  #"Bufo viridis", #NOTICE that iNaturalist uses "Bufotes viridis" as species name
  "Bufotes viridis",
  #"Bufo calamita", #NOTICE that iNaturalist uses "Epidalea calamita" as species name
  "Epidalea calamita",
  "Rana dalmatina",
  "Rana arvalis",
  "Hyla arborea",
...
df_g03bs$scientific_name2 <- df_g03bs$scientific_name
# replace with taxonomic names used by Spreybock et al. 2010
df_g03bs$scientific_name2 <- gsub("Bufotes viridis","Bufo viridis",df_g03bs$scientific_name2)
df_g03bs$scientific_name2 <- gsub("Epidalea calamita","Bufo calamita",df_g03bs$scientific_name2)
...
df_g01obs$dec_lat <- as.numeric(df_g01obs$latitude)
df_g01obs$dec_lon <- as.numeric(df_g01obs$longitude)
# copy the column with scientific names
df_g01obs$taxon.name <- df_g01obs$scientific_name
...
df_g01obs$taxon.name <- gsub("Epidalea calamita","Bufo calamita",df_g01obs$taxon.name)
#df_g01obs$taxon.name <- gsub("Pelophylax kl.","Pelophylax kl esculenta",df_g01obs$taxon.name)
df_g01obs$taxon.name <- gsub("Pelophylax esculentus","Pelophylax klesculenta",df_g01obs$taxon.name)
# check the unique names of Pelophylax
#unique(df_g01obs$taxon.name)[grepl("Pelophylax",unique(df_g01obs$taxon.name))]
...
jitlvl <- 0.017
# exclude records outside the area that is to be mapped
df_g01obs <- df_g01obs[(df_g01obs$dec_lon >= (set_swlng-2)),]
df_g01obs <- df_g01obs[(df_g01obs$dec_lon <= (set_nelng+2)),]
df_g01obs <- df_g01obs[(df_g01obs$dec_lat <= (set_nelat+2)),]
df_g01obs <- df_g01obs[(df_g01obs$dec_lat >= (set_swlat-2)),]
...
cl_nbg <- colfunc(nbg)
# define limits for plot boundaries
...
pthfigname02 <- paste0(wd00_wd09,"/",fnm02,".png")
#getwd()

if(bSaveFigures==T){
  ggsave(p,file=pthfigname02,
