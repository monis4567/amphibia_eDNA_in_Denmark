...
wd09 <- "/supma10_plots_from_R_analysis_for_fishes"
#paste together path
...
#smpls.Lf <- smpls[grepl("Loev",smpls$specs),]
# set working directory
setwd (wd00)
#getwd()
...
spl3 <- spl2
spl3.Lv <- spl3[grepl("Loev",spl3$specs),]
...
#dput(latspecnm)
list_of_fishes <- c("Perca_fluviatilis", "Thunnus_thynnus", "Abramis_brama", "Misgurnus_fossilis", 
                    "Anguilla_anguilla", "Esox_lucius", "Cyprinus_carpio", "Carassius_carassius",
                    "Scomber_scombrus", "Cobitis_taenia", 
                    "Oncorhynchus_gorbuscha", "Pleuronectes_platessa",
                    "Acipenser_baerii", "Clupea_harengus", 
                    "Rutilus_rutilus", "Platichthys_flesus", "Carassius_auratus", 
                    "Neogobius_melanostomus",
                    "Gadus_morhua", "Gasterosteus_aculeatus")

latspecnm <- list_of_fishes
...
#nrow(spl3.kl.fr)
spl3$NTC.CtdRn <- as.numeric(spl3$NTC.CtdRn)
spl3$NPC.CtdRn <- as.numeric(spl3$NPC.CtdRn)
spl3$NTC.CtdRn[is.na(spl3$NTC.CtdRn)] <- 0
spl3$NPC.CtdRn[is.na(spl3$NPC.CtdRn)] <- 0


spl3$unkn1.CtdRn <- as.numeric(spl3$unkn1.CtdRn)
spl3$unkn2.CtdRn <- as.numeric(spl3$unkn2.CtdRn)
spl3$unkn3.CtdRn <- as.numeric(spl3$unkn3.CtdRn)
spl3$unkn4.CtdRn <- as.numeric(spl3$unkn4.CtdRn)
spl3$unkn1.CtdRn[is.na(spl3$unkn1.CtdRn)] <- 0
spl3$unkn2.CtdRn[is.na(spl3$unkn2.CtdRn)] <- 0
spl3$unkn3.CtdRn[is.na(spl3$unkn3.CtdRn)] <- 0
spl3$unkn4.CtdRn[is.na(spl3$unkn4.CtdRn)] <- 0
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# Plot single capture locations - start
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
#remove NAs from vector
latspecnm <- latspecnm[!is.na(latspecnm)]
#latspecnm <- "Hyla_arborea"
...
for (spec.lat in latspecnm){
  ...
  # subset to disregard any Cq values above ct.cutoff
  sbs.spl3 <- sbs.spl3[sbs.spl3$NPC.CtdRn<=ct.cutoff,]
  sbs.spl3 <- sbs.spl3[sbs.spl3$NTC.CtdRn<=ct.cutoff,]
  sbs.spl3 <- sbs.spl3[sbs.spl3$unkn1.CtdRn<=ct.cutoff,]
  sbs.spl3 <- sbs.spl3[sbs.spl3$unkn2.CtdRn<=ct.cutoff,]
  sbs.spl3 <- sbs.spl3[sbs.spl3$unkn3.CtdRn<=ct.cutoff,]
  sbs.spl3 <- sbs.spl3[sbs.spl3$unkn4.CtdRn<=ct.cutoff,]
  ...
  sbs.spl3.ntc_npc_approv <- sbs.spl3[ which(sbs.spl3$NTC.CtdRn==0 & sbs.spl3$NPC.CtdRn>0), ]
  ...
  sbs.spl3.1or2pf <- sbs.spl3.ntc_npc_approv[ which(sbs.spl3.ntc_npc_approv$unkn1.CtdRn>0 
  ...
  sbs.spl3.2pf <- sbs.spl3.1or2pf[ which(sbs.spl3.1or2pf$unkn1.CtdRn>0 
                                         & sbs.spl3.1or2pf$unkn2.CtdRn>0), ]
  #count using the plyr-package
...
# subset to disregard any Cq values above ct.cutoff
spl3 <- spl3[spl3$NPC.CtdRn<=ct.cutoff,]
spl3 <- spl3[spl3$NTC.CtdRn<=ct.cutoff,]
spl3 <- spl3[spl3$unkn1.CtdRn<=ct.cutoff,]
spl3 <- spl3[spl3$unkn2.CtdRn<=ct.cutoff,]
spl3 <- spl3[spl3$unkn3.CtdRn<=ct.cutoff,]
spl3 <- spl3[spl3$unkn4.CtdRn<=ct.cutoff,]
#subset based on variable values - see: https://stackoverflow.com/questions/4935479/how-to-combine-multiple-conditions-to-subset-a-data-frame-using-or
# subset by when NTC is equal to NoCt and NPC is below ct.cut.off, and or NTC is equal to zero
spl3.ntc_npc_approv <- spl3[ which(spl3$NTC.CtdRn==0 & 
                        spl3$NPC.CtdRn>0), ]
#count using the plyr-package
approvK_smpl <- plyr::count(spl3.ntc_npc_approv, c("DLsamplno","abbr.nm"))

spl3.ntc_npc_approv$unkn1.CtdRn <- as.numeric(spl3.ntc_npc_approv$unkn1.CtdRn)
spl3.ntc_npc_approv$unkn2.CtdRn <- as.numeric(spl3.ntc_npc_approv$unkn2.CtdRn)
spl3.ntc_npc_approv$unkn1.CtdRn[is.na(spl3.ntc_npc_approv$unkn1.CtdRn)] <- 0
spl3.ntc_npc_approv$unkn1.CtdRn[is.na(spl3.ntc_npc_approv$unkn1.CtdRn)] <- 0
#subset based on variable values
# subset among the NPC and NTC approved replicate sets
#subset by when repl1 is below ct.cut.off and/or  when repl2 is below ct.cut.off
spl3.1or2pf <- spl3.ntc_npc_approv[ which(spl3.ntc_npc_approv$unkn1.CtdRn>0 
                                          | spl3.ntc_npc_approv$unkn2.CtdRn>0), ]
#count using the plyr-package
r1orr2pos.smpl <- plyr::count(spl3.1or2pf, c("DLsamplno", "abbr.nm"))
# subset among the NPC and NTC approved replicate sets with either 1 or 2 positive replicates
#subset by when both repl1 is below ct.cut.off and when repl2 is below ct.cut.off
spl3.2pf <- spl3.1or2pf[ which(spl3.1or2pf$unkn1.CtdRn>0 
                               & spl3.1or2pf$unkn2.CtdRn>0), ]
#count using the plyr-package
r2pos.smpl <- plyr::count(spl3.2pf, c("DLsamplno", "abbr.nm"))
#_______________________________________________________________________________
# The 'tot_smpl' data frame counts incorrect across multiple DL-sampling sites
# this does not take into account that some DL sampling sites have been 
# analysed for the same species more than once 
# instead the count of evaluations should be made across each individual test 
# set of 4 tubes
#_______________________________________________________________________________
# copy the data frame that holds the individual DL-samples per individual 
# set of 4 test tubes
spl4 <- spl3
# define columns to replace in
clEv  <-  c("NPC.CtdRn",
  "NTC.CtdRn",
  "unkn1.CtdRn",
  "unkn2.CtdRn",
...
spl4 <- spl4 %>% 
  dplyr::mutate_at(clEv, as.numeric)
# Replace NA on multiple columns by name
#https://sparkbyexamples.com/r-programming/replace-na-with-0-in-multiple-columns-in-r/#
spl4 <- spl4 %>% 
  dplyr::mutate_at(clEv, ~replace_na(.,0))
# make evaluations 
...
spl4$eval04 <- NA
spl4$eval04[which(spl4$NPC.CtdRn==0 | spl4$NTC.CtdRn>0)] <- "nonapprovK"
spl4$eval04[which(spl4$NPC.CtdRn>0 & spl4$NTC.CtdRn==0)] <- "truezerodetect"
...
spl5 <- spl4 %>%
  dplyr::mutate(n=rowSums(across(c(unkn1.CtdRn,unkn2.CtdRn,unkn3.CtdRn,unkn4.CtdRn), function(x) x>0))) %>%
  dplyr::mutate(eval05=ifelse(eval04=="truezerodetect",ifelse(n>1,"repl2p",ifelse(n>0,"repl1p",eval04)),eval04))

spl5.Ha <- spl5[grepl("Hyla",spl5$latspc),]
#unique(spl5.Ha$eval05)
#unique(spl5$eval05)
#_______________________________________________________________________________
#Rename the frequency column
...
#see this website for more about jitter on scatter plot
#https://thomasleeper.com/Rcourse/Tutorials/jitter.html
spl5$dec_lat.j <- jitter(spl5$dec_lat, 40.4)
...
spl5$abbr.nm <- dkspecs_to_latspecs$abbr.nm[match(spl5$specs,dkspecs_to_latspecs$Species_DK)]
...
amph_smpl03_df$eval04 <- NA
amph_smpl03_df$eval04[which(amph_smpl03_df$NPC.CtdRn==0 | amph_smpl03_df$NTC.CtdRn>0)] <- "nonapprovK"
amph_smpl03_df$eval04[which(amph_smpl03_df$NPC.CtdRn>0 & amph_smpl03_df$NTC.CtdRn==0)] <- "truezerodetect"

library(dplyr)
# for each row, n = count of the number of values from smpl01-04 which are >0
# we do this by applying the Boolean function x>0 to each column and then taking the sum
# then determine New_eval based on eval and n
amph_smpl03_df <- amph_smpl03_df %>%
  dplyr::mutate(n=rowSums(across(c(unkn1.CtdRn,unkn2.CtdRn,unkn3.CtdRn,unkn4.CtdRn), function(x) x>0))) %>%
  dplyr::mutate(eval05=ifelse(eval04=="truezerodetect",ifelse(n>1,"repl2p",ifelse(n>0,"repl1p",eval04)),eval04))

#use dplyr to group by name, and count per evaluation
tibl_as02 <- amph_smpl03_df %>% dplyr::group_by(abbr.nm) %>% dplyr::count(eval05)
...
# reorder columns
df_as05 <- df_as05[,c(1,2,4,3,5)]
#sum two columns with positive detections
df_as05$nrepl.pos <- rowSums(df_as05[,c("n.repl1p" ,"n.repl2p") ])
#remove the columns no longer needed
df_as05[ , c('n.repl1p', 'n.repl2p')] <- list(NULL)
...
#replace abbreviated species name with full latin species names
longNms <- dkspecs_to_latspecs$Species_Latin[match(df_as04$abbr.nm,dkspecs_to_latspecs$abbr.nm)]
#add back species abbreaviation
df_as05$abbr.nm <- c(longNms,"Total count")
...
library(tidyr)
# exclude the total count
df_as06 <- df_as06[!df_as06$abbr.nm=="Total count",]
...
df_as07$testres <- gsub("n.truezerodetect","no eDNA",df_as07$testres)
...
plt06 <- plt06 + geom_text(data=tibl_as07, aes(x = abbr.nm2, y = -8.95, 
...
amph_smpl02_df$pchsymb <- (amph_smpl02_df$eval05=="truezerodetect")*1
amph_smpl02_df$pchsymb <- (amph_smpl02_df$eval05=="repl2p")*1*21
...
amph_smpl04_df <- amph_smpl03_df[!amph_smpl03_df$eval05=="nonapprovK", ]
tot_smpl03_df <- tot_smpl[tot_smpl$nonapprovK==0, ]
#subset to only include positive detections
amph_smpl05_df <- amph_smpl04_df[grepl("repl",amph_smpl04_df$eval05),]
...
#are zero
amph_smpl03_df <- amph_smpl02_df[!amph_smpl02_df$eval05=="nonapprovK",]
...
amph_smpl03_df$latspc[grepl("lessonae",amph_smpl03_df$latspc)] <- "Pelophylax_sp"
amph_smpl03_df$latspc[grepl("Pelophylax_esculentus",amph_smpl03_df$latspc)]  <- "Pelophylax_sp"
amph_smpl03_df$latspc[grepl("ridibundus",amph_smpl03_df$latspc)] <- "Pelophylax_sp"
#
p01 <- ggplot(data = world) +
  geom_sf(color = "black", fill = "azure3") +
  #https://ggplot2.tidyverse.org/reference/position_jitter.html
...
#cl <- cbbPalette
# #variables for legend - not used
cl4 <- c(colfunc(length(unique(amph_smpl03_df$abbr.nm))))
ncl4 <- length(unique(amph_smpl03_df$abbr.nm))
#plot with abbreviated species names
p01 <- ggplot(data = world) +
  geom_sf(color = "black", fill = "azure3") +
  #https://ggplot2.tidyverse.org/reference/position_jitter.html
  ...
  geom_jitter(data = amph_smpl03_df, 
              ...
              size = 3.0) +
  #manually set the pch shape of the points
  scale_shape_manual(values=c(rep(21,nrow(amph_smpl03_df)))) +
  #set the color of the points
  #here it is black, and repeated the number of times
  #matching the number of species listed
...
amph_smpl03_df <- amph_smpl03_df[amph_smpl03_df$eval01=="nonapprovK", ]
#define input file
...
amph_smpl03_df$latspc <- df_dk_to_latnms01$Lat_spc_mn[match(amph_smpl03_df$specs,df_dk_to_latnms01$DK_comm_nm)]
#make a colour function
clt <- cl2
clt <- c(colfunc(length(unique(amph_smpl03_df$latspc))))
#count the number of species
noofspcsnms <- length(unique(amph_smpl03_df$latspc))
#subset the data frame
amph_smpl03_df <- amph_smpl03_df[(amph_smpl03_df$eval01=="nonapprovK"),]
#subset the data frame to exlude NAs
amph_smpl03_df <- amph_smpl03_df[!is.na(amph_smpl03_df$latspc),]

#plot with long species names
#https://statisticsglobe.com/create-legend-in-ggplot2-plot-in-r
#replace in latin species names
amph_smpl03_df$latspc2 <- gsub("_"," ",amph_smpl03_df$latspc)
...
amph_smpl03_df$eval01  <- amph_smpl03_df$eval05
#make a new set of colours for points
cl02 <- c("springgreen1","springgreen4","white","yellow")
#amph_smpl03_df$eval01
# plot all sampled sites where both controls 
# perform as expected - i.e. the true zero detect, and the 
# 1 pos repl, and 2 pos repl
...
p02
# you will have to change the legend for all legends
p02 <- p02 + labs(color='evaluation incl failed tests')
p02 <- p02 + labs(fill='evaluation incl failed tests')
p02 <- p02 + labs(shape='evaluation incl failed tests')
#  try a new plot
...
# i.e. subset by making sure the nonapproved controls 
#are zero
amph_smpl03.1_df <- amph_smpl03_df[!amph_smpl03_df$eval01=="nonapprovK",]
# begin the plot
p03 <- ggplot(data = world) +
  geom_sf(color = "black", fill = "azure3") +
  #https://ggplot2.tidyverse.org/reference/position_jitter.html
  # use 'geom_jitter' instead of 'geom_point' 
  geom_jitter(data = amph_smpl03.1_df, 
              aes(x = dec_lon, y = dec_lat,
                  fill=eval01,
                  color=eval01,
                  shape=eval01),
              ...
              size = 3) +
  #manually set the pch shape of the points
  scale_shape_manual(values=c(rep(21,length(unique(amph_smpl03.1_df$eval01))))) +
  #set the color of the points
  #here it is black, and repeated the number of times
  #matching the number of species listed
  scale_color_manual(values=c(rep("black",length(unique(amph_smpl03.1_df$eval01))))) +
  #set the color of the points
  #use alpha to scale the intensity of the color
  scale_fill_manual(values=alpha(
    c(cl03),
...
#p03
# you will have to change the legend for all legends
p03 <- p03 + labs(color='evaluation excl failed tests')
p03 <- p03 + labs(fill='evaluation excl failed tests')
p03 <- p03 + labs(shape='evaluation excl failed tests')
...
#subset to exclude all NonApproved controls
df_as04 <- df_as03[df_as03$eval05=="nonapprovK", ]
#copy the data frame
df_as05 <- df_as04
#make the column with numbers for symbols a factor column
...
outfl1 = "out09_01b_DL_records_fishes_Denmark.csv"
...
df_as06 <- apply(df_as03,2,as.character)
# write a table
...
df_as03.1 <- as.data.frame(df_as03)
...
df_as03.1$latspc2 <- gsub("_"," ",df_as03.1$latspc)
# get day number for each sample collection date
df_as03.1$NumDays  <- df_as09$NumDays[match(df_as03.1$DLsamplno,df_as09$DLsamplno)]
# copy the column with day number
df_as03.1$dayno <- df_as03.1$NumDays
# make repl1p and repl2p identical as 'eDNA_detected' by substituting
df_as03.1$eval05 <- gsub("repl.*","eDNA detected",df_as03.1$eval05)
df_as03.1$eval05 <- gsub("nonapprovK","failed test",df_as03.1$eval05)
df_as03.1$eval05 <- gsub("truezerodetect","no eDNA",df_as03.1$eval05)

# make a ggplot
plt09 <- ggplot(df_as03.1, aes(x=latspc2, y=dayno)) +
  # use points with fill based on evaluation, and jitter their position
  geom_point(aes(fill=eval05), size=3, shape=21,
...
df_as10 <- df_as03.1[df_as03.1$eval05!="failed test",]
...
df_as10$eval05 <- gsub("repl.*","eDNA detected",df_as10$eval05)
# make a ggplot
...
df_as10 <- df_as10[df_as10$eval05=="eDNA detected",]
# make a ggplot
plt09 <- ggplot(df_as10, aes(x=latspc2, y=dayno)) +
  # use points with fill based on evaluation, and jitter their position
  geom_point(aes(fill=eval05), size=3, shape=21,
...
# copy the data frame
df_as06.1 <- as.data.frame(df_as03.1)
df_as06.1$eval05 <- gsub("nonapprovK","failed test",df_as06.1$eval05)
df_as06.1$eval05 <- gsub("truezerodetect","no eDNA",df_as06.1$eval05)
df_as06.1$eval05 <- gsub("repl.*","eDNA detected",df_as06.1$eval05)
#match to get area covered by water body collected
df_as06.1$Areal_m2 <- hc$Areal_m2[match(df_as06.1$DLsamplno,hc$DL_No)]
# make the area value numeric
df_as06.1$Areal_m2 <- as.numeric(df_as06.1$Areal_m2)
# exclude sampling that do not have an area for the water body collected
df_as10 <- df_as06.1[!is.na(df_as06.1$Areal_m2),]
...
df_as12 <- df_as10[df_as10$eval05!="failed test",]
# make a scattered box plot for species
...
library(ggplot2)
# substitute for positive replicates
df_as03.1$eval05 <- gsub("repl.*","eDNA detected",df_as03.1$eval05)
# match to get area of water body sampled
df_as03.1$Areal_m2 <- df_as10$Areal_m2[match(df_as03.1$DLsamplno,df_as10$DLsamplno)]
# replace NAs with zero
df_as03.1$Areal_m2[is.na(df_as03.1$Areal_m2)] <- 0
# remove sample locations without an area
...
# subset data frame
df_as11 <- df_as03.1[df_as03.1$eval05=="eDNA detected",]
df_as11 <- df_as03.1[!df_as03.1$eval05=="failed test",]
# substitute in latin species names
df_as11$latspc2 <- gsub("Pelophylax esculentus","Pelophylax sp",df_as11$latspc2)
...
plt13 <- ggplot(df_as11, aes(x=latspc2, y=dayno)) +
  ...
  geom_point(data=df_as11,aes(fill=eval05),position=position_jitter(width=0.2, height=0.1),
...
# replace the matches with 'lessonae'
df_as11$latspc2[grepl("lessonae",df_as11$latspc2)] <- "Pelophylax sp"
# substitute "Pelophylax esculentus"
df_as11$latspc2 <- gsub("Pelophylax esculentus","Pelophylax sp",df_as11$latspc2) 
# -------- make data for date labels  ------- 
...
# Med en søgning på den kombinerede csv fil i unix:
# $  grep randtudse outfile07_merged_csv_files_from_mxpro.csv | grep ';eDNA;' | grep -v NoCt
# Kan få de linjer hvori strandtudsen blev sporet med eDNA 
# med
# $ grep randtudse outfile07_merged_csv_files_from_mxpro.csv | grep ';eDNA;' | grep -v NoCt | cut -d';' -f9 | sort | uniq
# Kan jeg få fat i de DL numrer hvor 
# strandtudse blev sporet med eDNA:
# DL2017048 forfejlet test
# DL2018136 forfejlet test
# DL2018009 sand positiv
# DL2018005 sand positiv
# DL2018050 forfejlet test
# DL2018036 forfejlet test
# DL2018128 forfejlet test
# DL2019042 forfejlet test
# DL2019065 sand positiv
# 
# Det er kun DL2018009, DL2018005 og DL2019065 der giver en sand positive sporing af eDNA
# DL2018005 er fra Moen_loll_falst og lader til at være ok. Det er oså det eneste punkt med strandtudse , der er inkluderet
# DL2018009 er testet igen af mig selv i 8 replikater, uden at jeg kunne genfinde denne sporinge af strandtudse.
# Denne prøve for strandtudse er derfor fjernet ovenover
# Ligeledes for DL2019065, som jeg heller ikke kunne finde i de ekstra 8 replikater. Hvorfor den også er ekskluderet.
